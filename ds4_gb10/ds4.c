...
static bool __attribute__((unused)) flashmoe_pack_selected_experts(
    ...
    out_pack->up_expert_bytes = up_bytes;
    out_pack->down_expert_bytes = down_bytes;
    out_pack->gate_row_bytes = gate_rb;
    out_pack->up_row_bytes = up_rb;
    out_pack->down_row_bytes = down_rb;
    return true;
}
...
static bool flashmoe_upload_selected_pack(
        ...
        uint32_t                          n_pairs,
        ds4_gpu_tensor                   *gate_reuse,
        ds4_gpu_tensor                   *up_reuse,
        ds4_gpu_tensor                   *down_reuse,
    ...
    *down_w = NULL;
    *selected_gpu = NULL;
    if (!pack || pack->active_count == 0 || n_pairs == 0) return false;

    const uint64_t gate_bytes = (uint64_t)pack->active_count * pack->gate_expert_bytes;
    const uint64_t up_bytes = (uint64_t)pack->active_count * pack->up_expert_bytes;
    const uint64_t down_bytes = (uint64_t)pack->active_count * pack->down_expert_bytes;
    if (gate_reuse) {
        *gate_w = ds4_gpu_tensor_view(gate_reuse, 0, gate_bytes);
    } else {
        *gate_w = ds4_gpu_tensor_alloc(gate_bytes);
    }
    if (up_reuse) {
        *up_w = ds4_gpu_tensor_view(up_reuse, 0, up_bytes);
    } else {
        *up_w = ds4_gpu_tensor_alloc(up_bytes);
    }
    if (down_reuse) {
        *down_w = ds4_gpu_tensor_view(down_reuse, 0, down_bytes);
    } else {
        *down_w = ds4_gpu_tensor_alloc(down_bytes);
    }
...
static bool flashmoe_stage_selected_pack_blob(
        const ds4_flashmoe_selected_pack *pack,
        uint8_t                          *blob_dst,
        uint64_t                          blob_dst_bytes,
        double                           *layout_s_out) {
    if (!pack || !blob_dst || pack->active_count == 0) return false;
    const uint64_t blob_stride = pack->gate_expert_bytes +
                                 pack->up_expert_bytes +
                                 pack->down_expert_bytes;
    const uint64_t total_bytes = (uint64_t)pack->active_count * blob_stride;
    if (blob_dst_bytes < total_bytes) return false;
    const bool timing = flashmoe_timing_enabled();
    const double t0 = timing ? now_sec() : 0.0;
    for (uint32_t i = 0; i < pack->active_count; i++) {
        uint8_t *dst = blob_dst + (uint64_t)i * blob_stride;
        memcpy(dst,
               pack->gate_bytes + (uint64_t)i * pack->gate_expert_bytes,
               (size_t)pack->gate_expert_bytes);
        memcpy(dst + pack->gate_expert_bytes,
               pack->up_bytes + (uint64_t)i * pack->up_expert_bytes,
               (size_t)pack->up_expert_bytes);
        memcpy(dst + pack->gate_expert_bytes + pack->up_expert_bytes,
               pack->down_bytes + (uint64_t)i * pack->down_expert_bytes,
               (size_t)pack->down_expert_bytes);
    }
    if (timing && layout_s_out) *layout_s_out += now_sec() - t0;
    return true;
}

static bool flashmoe_upload_selected_blob_pack(
        const ds4_flashmoe_selected_pack *pack,
        uint32_t                          n_pairs,
        const uint8_t                    *blob_host,
        ds4_gpu_tensor                   *blob_reuse,
        ds4_gpu_tensor                   *selected_reuse,
        ds4_gpu_tensor                  **blob_w,
        ds4_gpu_tensor                  **selected_gpu,
        double                          *tensor_upload_s_out,
        double                          *selected_write_s_out) {
    *blob_w = NULL;
    *selected_gpu = NULL;
    if (!pack || !blob_host || !blob_reuse || pack->active_count == 0 || n_pairs == 0) return false;
    const uint64_t blob_stride = pack->gate_expert_bytes +
                                 pack->up_expert_bytes +
                                 pack->down_expert_bytes;
    const uint64_t blob_bytes = (uint64_t)pack->active_count * blob_stride;
    *blob_w = ds4_gpu_tensor_view(blob_reuse, 0, blob_bytes);
    *selected_gpu = selected_reuse ? selected_reuse
                                   : ds4_gpu_tensor_alloc((uint64_t)n_pairs * sizeof(pack->selected_local[0]));
    if (!*blob_w || !*selected_gpu) return false;
    const bool timing = flashmoe_timing_enabled();
    const double t_blob0 = timing ? now_sec() : 0.0;
    if (ds4_gpu_tensor_write(*blob_w, 0, blob_host, blob_bytes) == 0) return false;
    if (timing && tensor_upload_s_out) *tensor_upload_s_out += now_sec() - t_blob0;
    const double t_sel0 = timing ? now_sec() : 0.0;
    if (ds4_gpu_tensor_write(*selected_gpu, 0, pack->selected_local,
                             (uint64_t)n_pairs * sizeof(pack->selected_local[0])) == 0) {
        return false;
...
static bool flashmoe_decode_blob_slot_enabled(void) {
    static int cache = -1;
    if (cache == -1) {
        const char *env = getenv("DS4_FLASHMOE_DECODE_BLOB_SLOT");
        cache = (!env || !env[0] || strcmp(env, "0") != 0) ? 1 : 0;
    }
    return cache != 0;
}

static bool flashmoe_decode_gpu_hit_lookup_enabled(void) {
    static int cache = -1;
    if (cache == -1) {
        const char *env = getenv("DS4_FLASHMOE_GPU_HIT_LOOKUP");
        cache = (env && env[0] && strcmp(env, "0") != 0) ? 1 : 0;
    }
    return cache != 0;
}

static bool flashmoe_router_event_sync_enabled(void) {
    static int cache = -1;
    if (cache == -1) {
        const char *env = getenv("DS4_FLASHMOE_ROUTER_EVENT_SYNC");
        cache = (env && env[0] && strcmp(env, "0") != 0) ? 1 : 0;
    }
    return cache != 0;
}
...
static const char *flashmoe_decode_layer_trace_path(void) {
    static const char *cache = (const char *)(uintptr_t)1u;
    if (cache != (const char *)(uintptr_t)1u) return cache;
...
static uint32_t flashmoe_decode_slot_count(void) {
    ...
    uint32_t count = DS4_N_EXPERT_USED;
    if (env && env[0]) {
        char *end = NULL;
        unsigned long parsed = strtoul(env, &end, 10);
...
static bool flashmoe_write_slot_tensor_run(ds4_gpu_tensor *base,
                                           uint64_t start_slot,
                                           uint64_t slot_bytes,
                                           uint32_t slot_count,
                                           const void *src) {
...
static DS4_MAYBE_UNUSED void layer_routed_moe_batch_flashmoe_selected(
        float                    *moe,
        float                    *mid_out,
        const ds4_layer_weights  *layer,
        const float              *norm,
    ...
    if (down_in_dim % QK_K != 0) ds4_die("Q2_K expert input is not QK_K aligned");
    if (expert_out_dim != down_in_dim || down_out_dim != DS4_N_EMBD) {
        ds4_die("FlashMoE selected expert tensor layout is unexpected");
    }
    ...
    for (uint32_t t = 0; t < n_tok; t++) {
        ...
        for (uint32_t slot = 0; slot < DS4_N_EXPERT_USED; slot++) {
            const uint32_t pair_id = t * DS4_N_EXPERT_USED + slot;
            const int expert = selected[pair_id];
            pairs[pair_id] = (ds4_expert_pair){ .token = t, .slot = slot };
    ...
    float *mid = mid_out ? mid_out : xmalloc((size_t)total_pairs * expert_out_dim * sizeof(mid[0]));
    matvec_iq2_xxs_batch_mid_ctx mid_ctx = {
        .mid = mid,
        .xq = xq,
        .pairs = pairs,
...
struct ds4_gpu_graph {
    ...
    ds4_gpu_tensor *flashmoe_blob_w;
    ...
    uint64_t flashmoe_blob_host_bytes;
    ...
    double flashmoe_decode_router_kernel_s;
    double flashmoe_decode_readback_wait_s;
    double flashmoe_decode_readback_copy_s;
    double flashmoe_decode_slot_lookup_s;
    double flashmoe_decode_blob_read_s;
    double flashmoe_decode_selected_write_s;
    double flashmoe_decode_slot_fill_s;
    double flashmoe_decode_router_kernel_max_s;
    double flashmoe_decode_readback_wait_max_s;
    double flashmoe_decode_readback_copy_max_s;
    double flashmoe_decode_slot_lookup_max_s;
    double flashmoe_decode_blob_read_max_s;
    double flashmoe_decode_selected_write_max_s;
    double flashmoe_decode_slot_fill_max_s;
    uint32_t flashmoe_decode_router_kernel_max_layer;
    uint32_t flashmoe_decode_readback_wait_max_layer;
    uint32_t flashmoe_decode_readback_copy_max_layer;
    uint32_t flashmoe_decode_slot_lookup_max_layer;
    uint32_t flashmoe_decode_blob_read_max_layer;
    uint32_t flashmoe_decode_selected_write_max_layer;
    uint32_t flashmoe_decode_slot_fill_max_layer;
...
static void flashmoe_decode_stage_max_update(
        double *max_s,
        uint32_t *max_layer,
        double sample_s,
        uint32_t il) {
...
static void flashmoe_decode_layer_trace_append(
        ...
        uint32_t active_count,
        uint32_t miss_count,
        double router_kernel_s,
        double readback_wait_s,
        double readback_copy_s,
        double slot_lookup_s,
        double blob_read_s,
        double selected_write_s,
        double slot_fill_s,
        double routed_kernel_s,
        double other_s) {
    const char *path = flashmoe_decode_layer_trace_path();
    if (!path || !path[0]) return;
    FILE *fp = fopen(path, "a+");
    if (!fp) return;
    if (fseek(fp, 0, SEEK_END) == 0) {
        ...
        if (size == 0) {
            fprintf(fp,
                    "ordinal,layer,slot_count,active_count,miss_count,router_kernel_ms,readback_wait_ms,readback_copy_ms,slot_lookup_ms,blob_read_ms,selected_write_ms,slot_fill_ms,routed_kernel_ms,other_ms\n");
        }
    }
    fprintf(fp,
            "%" PRIu64 ",%u,%u,%u,%u,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
            ordinal,
            il,
            slot_count,
            active_count,
            miss_count,
            router_kernel_s * 1000.0,
            readback_wait_s * 1000.0,
            readback_copy_s * 1000.0,
            slot_lookup_s * 1000.0,
            blob_read_s * 1000.0,
            selected_write_s * 1000.0,
            slot_fill_s * 1000.0,
            routed_kernel_s * 1000.0,
            other_s * 1000.0);
    fclose(fp);
}

static bool flashmoe_decode_ensure_layer_cache(
        ds4_gpu_graph *g,
        const ds4_layer_weights *layer,
        uint32_t il) {
    ...
    if (!g->flashmoe_decode_blob_slots[il] ||
        ...
        const uint64_t up_rb = routed_expert_row_bytes(layer->ffn_up_exps);
        const uint64_t down_rb = routed_expert_row_bytes(layer->ffn_down_exps);
        const uint64_t gate_exp_bytes = gate_rb * layer->ffn_gate_exps->dim[1];
        const uint64_t up_exp_bytes = up_rb * layer->ffn_up_exps->dim[1];
        const uint64_t down_exp_bytes = down_rb * layer->ffn_down_exps->dim[1];
        ...
        if (!g->flashmoe_decode_expert_slot_gpu[il]) {
            g->flashmoe_decode_expert_slot_gpu[il] =
...
static bool flashmoe_decode_prepare_slot_cache(
        ...
        uint32_t                *active_count_out,
        uint32_t                *miss_count_out,
        double                  *host_s_out,
        double                  *slot_lookup_s_out,
        double                  *blob_read_s_out,
        double                  *upload_s_out) {
    if (!flashmoe_decode_gpu_cache_enabled()) {
        return false;
    }
    if (!flashmoe_decode_ensure_layer_cache(g, layer, il)) return false;
    ...
    uint32_t active_count = 0;
    int32_t active_slots[DS4_N_EXPERT_USED];
    uint16_t miss_experts[DS4_N_EXPERT_USED];
    uint32_t miss_slots[DS4_N_EXPERT_USED];
    uint32_t miss_count = 0;
    uint64_t gate_bytes = 0, up_bytes = 0, down_bytes = 0;
    uint64_t gate_rb = 0, up_rb = 0, down_rb = 0;
    char ferr[256];
    double host_s = 0.0;
    double slot_lookup_s = 0.0;
    double blob_read_s = 0.0;
    double upload_s = 0.0;
    const bool timing = flashmoe_timing_enabled();
    const double t_lookup0 = timing ? now_sec() : 0.0;
    ...
    for (uint32_t i = 0; i < active_count; i++) {
        ...
        if (slot_index >= 0) {
            ...
            if (!(slot < slot_count &&
                  g->flashmoe_decode_slot_valid[il][slot] &&
                  g->flashmoe_decode_slot_expert[il][slot] == (int)active_global[i])) {
                g->flashmoe_decode_expert_slot[il][active_global[i]] = -1;
                slot_index = -1;
            }
        }
        if (slot_index >= 0) {
        ...
        if (slot_index < 0) {
            ...
            for (uint32_t probe = 0; probe < slot_count; probe++) {
                ...
                if (!already_used && !g->flashmoe_decode_slot_valid[il][probe]) {
                    slot = probe;
                    break;
                }
            }
            ...
            if (g->flashmoe_decode_slot_valid[il][slot]) {
                const int old_expert = g->flashmoe_decode_slot_expert[il][slot];
                if (old_expert >= 0 && old_expert < DS4_N_EXPERT) {
                    g->flashmoe_decode_expert_slot[il][old_expert] = -1;
                }
            }
            miss_experts[miss_count] = active_global[i];
            miss_slots[miss_count] = slot;
            miss_count++;
            slot_index = (int32_t)slot;
        }
        active_slots[i] = slot_index;
        used_slots[used_count++] = (uint32_t)slot_index;
    }

    if (miss_count != 0) {
        const double t_blob0 = timing ? now_sec() : 0.0;
        if (timing) slot_lookup_s += t_blob0 - t_lookup0;
        for (uint32_t i = 1; i < miss_count; i++) {
            const uint16_t expert = miss_experts[i];
            const uint32_t slot = miss_slots[i];
            uint32_t j = i;
            while (j > 0 && miss_slots[j - 1] > slot) {
                miss_experts[j] = miss_experts[j - 1];
                miss_slots[j] = miss_slots[j - 1];
                j--;
            }
            miss_experts[j] = expert;
            miss_slots[j] = slot;
        }
        if (ds4_flashmoe_runtime_load_selected_blobs((uint16_t)il,
                                                     ...
                                                     sizeof(ferr)) != 0) {
            fprintf(stderr,
                    "ds4: FlashMoE backend failed to batch-load decode blobs layer=%u misses: %s\n",
                    il,
                    ferr[0] ? ferr : "unknown error");
            return false;
        }
        const double t_upload0 = timing ? now_sec() : 0.0;
        if (timing) {
            blob_read_s += t_upload0 - t_blob0;
            host_s += t_upload0 - t_lookup0;
        }
        uint32_t run_start = 0;
        while (run_start < miss_count) {
            uint32_t run_end = run_start + 1u;
            while (run_end < miss_count &&
                   miss_slots[run_end] == miss_slots[run_end - 1] + 1u) {
                run_end++;
            }
            const uint32_t run_len = run_end - run_start;
            if (!flashmoe_write_slot_tensor_run(g->flashmoe_decode_blob_slots[il],
                                                miss_slots[run_start],
                                                blob_stride,
                                                run_len,
        ...
        if (timing) {
            g->flashmoe_decode_used = true;
            g->flashmoe_decode_misses += miss_count;
        ...
        slot_lookup_s += now_sec() - t_lookup0;
        host_s += slot_lookup_s;
    }
    ...
    if (up_expert_bytes) *up_expert_bytes = up_bytes;
    if (gate_row_bytes) *gate_row_bytes = gate_rb;
    ...
    if (miss_count_out) *miss_count_out = miss_count;
    if (host_s_out) *host_s_out = host_s;
    if (slot_lookup_s_out) *slot_lookup_s_out = slot_lookup_s;
    if (blob_read_s_out) *blob_read_s_out = blob_read_s;
    if (upload_s_out) *upload_s_out = upload_s;
    return true;
}

static bool flashmoe_decode_run_stable_path(
        ds4_gpu_graph           *g,
        const ds4_layer_weights *layer,
        uint32_t                 il,
        const int               *selected,
        double                  *host_s_accum,
        double                  *upload_s_accum,
        double                  *kernel_s_accum) {
    ...
    if (ds4_gpu_tensor_fill_f32(g->routed_gate, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_mid_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_up, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_mid_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_mid, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_mid_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_down, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_out_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_out, 0.0f, routed_out_dim) == 0) {
        goto cleanup;
    }
    ...
    ok = ds4_gpu_routed_moe_one_external_tensor(g->routed_out,
                                                ...
                                                DS4_SWIGLU_CLAMP_EXP,
                                                g->ffn_norm) != 0;
    if (flashmoe_timing_enabled() && kernel_s_accum) *kernel_s_accum += now_sec() - t_kernel0;
...
static bool metal_graph_decode_routed_flashmoe(
        ...
        double                 *readback_wait_s_out,
        double                 *readback_copy_s_out,
        double                 *host_s_out,
        double                 *slot_lookup_s_out,
        double                 *blob_read_s_out,
        double                 *selected_write_s_out,
        double                 *slot_fill_s_out,
        double                 *upload_s_out,
        double                 *kernel_s_out,
        uint32_t               *slot_count_out,
        uint32_t               *active_count_out,
    ...
    double readback_wait_s = 0.0;
    double readback_copy_s = 0.0;
    double host_s = 0.0;
    double slot_lookup_s = 0.0;
    double blob_read_s = 0.0;
    double upload_s = 0.0;
    ...
    const bool timing = flashmoe_timing_enabled();
    if (flashmoe_decode_blob_slot_enabled() &&
        flashmoe_decode_gpu_hit_lookup_enabled() &&
        ...
        const double t_sync0 = timing ? now_sec() : 0.0;
        if (use_router_event_sync) {
            if (!g->flashmoe_router_ready_ev) {
                g->flashmoe_router_ready_ev = ds4_gpu_event_create();
                if (!g->flashmoe_router_ready_ev) goto cleanup;
            }
            if (ds4_gpu_event_record(g->flashmoe_router_ready_ev) == 0 ||
                ds4_gpu_event_synchronize(g->flashmoe_router_ready_ev) == 0) {
                goto cleanup;
            }
        } else {
            if (ds4_gpu_synchronize() == 0) goto cleanup;
        }
    ...
    if (!gpu_all_hit_fast) {
        ...
        const double t_read0 = timing ? now_sec() : 0.0;
        if (ds4_gpu_tensor_read(g->router_selected, 0, selected, (uint64_t)DS4_N_EXPERT_USED * sizeof(selected[0])) == 0) goto cleanup;
        if (timing) {
            readback_copy_s += now_sec() - t_read0;
            readback_s = readback_wait_s + readback_copy_s;
        }
    ...
    if (!flashmoe_decode_blob_slot_enabled()) {
        ok = flashmoe_decode_run_stable_path(g,
                                             layer,
                                             il,
                                             selected,
                                             &host_s,
    ...
    if (gpu_all_hit_fast) {
        ...
        if (flashmoe_timing_enabled()) {
            selected_write_s += now_sec() - t_sel0;
            upload_s += selected_write_s;
            slot_fill_s = upload_s - selected_write_s;
        }
    }
    const uint64_t routed_mid_dim = layer->ffn_gate_exps->dim[1];
    const uint64_t routed_out_dim = layer->ffn_down_exps->dim[1];
    if (ds4_gpu_tensor_fill_f32(g->routed_gate, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_mid_dim) == 0 ||
        ...
        ds4_gpu_tensor_fill_f32(g->routed_mid, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_mid_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_down, 0.0f, (uint64_t)DS4_N_EXPERT_USED * routed_out_dim) == 0 ||
        ds4_gpu_tensor_fill_f32(g->routed_out, 0.0f, routed_out_dim) == 0) goto cleanup;
    const double t_kernel0 = flashmoe_timing_enabled() ? now_sec() : 0.0;
    ok = ds4_gpu_routed_moe_one_external_blob_slots_tensor(g->routed_out,
                                                           g->routed_gate,
                                                           g->routed_up,
                                                           g->routed_mid,
                                                           g->routed_down,
...
cleanup:
    ...
    if (readback_wait_s_out) *readback_wait_s_out = readback_wait_s;
    if (readback_copy_s_out) *readback_copy_s_out = readback_copy_s;
    if (host_s_out) *host_s_out = host_s;
    if (slot_lookup_s_out) *slot_lookup_s_out = slot_lookup_s;
    if (blob_read_s_out) *blob_read_s_out = blob_read_s;
    if (upload_s_out) *upload_s_out = upload_s;
    if (selected_write_s_out) *selected_write_s_out = selected_write_s;
    if (slot_fill_s_out) *slot_fill_s_out = slot_fill_s;
...
static bool metal_graph_prefill_routed_flashmoe(
    ...
    int *selected = g->flashmoe_selected_host;
    ds4_flashmoe_selected_pack pack;
    ds4_gpu_tensor *blob_w = NULL, *selected_gpu = NULL;
    bool ok = false;
    const bool timing = flashmoe_timing_enabled();
    ...
    if (!flashmoe_stage_selected_pack_blob(&pack,
                                           g->flashmoe_blob_host,
                                           g->flashmoe_blob_host_bytes,
                                           &t_layout)) goto cleanup;
    if (timing) t_read = t_selected_readback + t_layout + t_blob_read;
    if (!flashmoe_upload_selected_blob_pack(&pack,
                                            (uint32_t)selected_elems,
                                            g->flashmoe_blob_host,
                                            g->flashmoe_blob_w,
                                            g->flashmoe_selected_gpu,
    ...
    ok = ds4_gpu_routed_moe_batch_external_blob_tensor(g->batch_routed_out,
                                                       g->batch_routed_gate,
                                                       g->batch_routed_up,
                                                       g->batch_routed_mid,
                                                       g->batch_routed_down,
...
cleanup:
    if (selected_gpu != g->flashmoe_selected_gpu) ds4_gpu_tensor_free(selected_gpu);
    if (blob_w != g->flashmoe_blob_w) ds4_gpu_tensor_free(blob_w);
    flashmoe_selected_pack_free(&pack);
    return ok;
}
...
static void metal_graph_free(ds4_gpu_graph *g) {
    ds4_gpu_host_free_pinned(g->flashmoe_down_host);
    ds4_gpu_host_free_pinned(g->flashmoe_up_host);
    ds4_gpu_host_free_pinned(g->flashmoe_gate_host);
    ds4_gpu_host_free_pinned(g->flashmoe_blob_host);
    ...
    ds4_gpu_tensor_free(g->flashmoe_blob_w);
    ds4_gpu_tensor_free(g->flashmoe_down_w);
    ds4_gpu_tensor_free(g->flashmoe_up_w);
    ds4_gpu_tensor_free(g->flashmoe_gate_w);
...
static bool metal_graph_alloc_raw_cap(
    ...
    uint64_t flashmoe_blob_bytes_max = 0;
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if ((uint64_t)DS4_N_EXPERT * up_exp_bytes > flashmoe_up_bytes_max) {
            flashmoe_up_bytes_max = (uint64_t)DS4_N_EXPERT * up_exp_bytes;
        }
        if ((uint64_t)DS4_N_EXPERT * down_exp_bytes > flashmoe_down_bytes_max) {
            flashmoe_down_bytes_max = (uint64_t)DS4_N_EXPERT * down_exp_bytes;
        }
        if ((uint64_t)DS4_N_EXPERT * blob_exp_bytes > flashmoe_blob_bytes_max) {
            flashmoe_blob_bytes_max = (uint64_t)DS4_N_EXPERT * blob_exp_bytes;
        }
    }
    ...
    g->flashmoe_blob_w = ds4_gpu_tensor_alloc(flashmoe_blob_bytes_max);
    g->flashmoe_gate_w = ds4_gpu_tensor_alloc(flashmoe_gate_bytes_max);
    g->flashmoe_up_w = ds4_gpu_tensor_alloc(flashmoe_up_bytes_max);
    g->flashmoe_down_w = ds4_gpu_tensor_alloc(flashmoe_down_bytes_max);
    g->flashmoe_blob_w_bytes = flashmoe_blob_bytes_max;
    ...
    g->flashmoe_selected_host = (int *)ds4_gpu_host_alloc_pinned((uint64_t)flashmoe_selected_pairs_cap * sizeof(g->flashmoe_selected_host[0]));
    g->flashmoe_selected_local_host = xmalloc((size_t)flashmoe_selected_pairs_cap * sizeof(g->flashmoe_selected_local_host[0]));
    g->flashmoe_blob_host = (uint8_t *)ds4_gpu_host_alloc_pinned(flashmoe_blob_bytes_max);
    g->flashmoe_gate_host = (uint8_t *)ds4_gpu_host_alloc_pinned(flashmoe_gate_bytes_max);
    g->flashmoe_up_host = (uint8_t *)ds4_gpu_host_alloc_pinned(flashmoe_up_bytes_max);
    g->flashmoe_down_host = (uint8_t *)ds4_gpu_host_alloc_pinned(flashmoe_down_bytes_max);
    g->flashmoe_selected_host_cap = flashmoe_selected_pairs_cap;
    g->flashmoe_selected_local_cap = flashmoe_selected_pairs_cap;
    g->flashmoe_blob_host_bytes = flashmoe_blob_bytes_max;
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
    ...
    double decode_readback_wait_s = 0.0;
    double decode_readback_copy_s = 0.0;
    double decode_host_s = 0.0;
    double decode_slot_lookup_s = 0.0;
    double decode_blob_read_s = 0.0;
    double decode_upload_s = 0.0;
    double decode_selected_write_s = 0.0;
    double decode_slot_fill_s = 0.0;
    double decode_kernel_s = 0.0;
    uint32_t decode_slot_count = 0;
    uint32_t decode_active_count = 0;
    uint32_t decode_miss_count = 0;
    ...
    if (ok) {
        if (ds4_flashmoe_runtime_ready()) {
            ok = metal_graph_decode_routed_flashmoe(g,
                                                    ...
                                                    il,
                                                    &decode_readback_s,
                                                    &decode_readback_wait_s,
                                                    &decode_readback_copy_s,
                                                    &decode_host_s,
                                                    &decode_slot_lookup_s,
                                                    &decode_blob_read_s,
                                                    &decode_selected_write_s,
                                                    &decode_slot_fill_s,
                                                    &decode_upload_s,
                                                    &decode_kernel_s,
                                                    &decode_slot_count,
...
#undef DS4_METAL_PROFILE_DECODE_STAGE
    if (flashmoe_timing_enabled() && ds4_flashmoe_runtime_ready()) {
        ...
        g->flashmoe_decode_router_kernel_s += decode_router_s;
        g->flashmoe_decode_readback_wait_s += decode_readback_wait_s;
        g->flashmoe_decode_readback_copy_s += decode_readback_copy_s;
        g->flashmoe_decode_slot_lookup_s += decode_slot_lookup_s;
        g->flashmoe_decode_blob_read_s += decode_blob_read_s;
        g->flashmoe_decode_selected_write_s += decode_selected_write_s;
        g->flashmoe_decode_slot_fill_s += decode_slot_fill_s;
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_router_kernel_max_s,
                                         &g->flashmoe_decode_router_kernel_max_layer,
                                         decode_router_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_readback_wait_max_s,
                                         &g->flashmoe_decode_readback_wait_max_layer,
                                         decode_readback_wait_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_readback_copy_max_s,
                                         &g->flashmoe_decode_readback_copy_max_layer,
                                         decode_readback_copy_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_slot_lookup_max_s,
                                         &g->flashmoe_decode_slot_lookup_max_layer,
                                         decode_slot_lookup_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_blob_read_max_s,
                                         &g->flashmoe_decode_blob_read_max_layer,
                                         decode_blob_read_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_selected_write_max_s,
                                         &g->flashmoe_decode_selected_write_max_layer,
                                         decode_selected_write_s,
                                         il);
        flashmoe_decode_stage_max_update(&g->flashmoe_decode_slot_fill_max_s,
                                         &g->flashmoe_decode_slot_fill_max_layer,
                                         decode_slot_fill_s,
                                         il);
        ...
        flashmoe_decode_layer_trace_append(g->flashmoe_decode_layers,
                                           il,
                                           decode_slot_count,
                                           decode_active_count,
                                           decode_miss_count,
                                           ...
                                           decode_slot_fill_s,
                                           decode_kernel_s,
                                           total > accounted ? total - accounted : 0.0);
    }
    if (ok) {
...
static int generate_metal_graph_raw_swa(
    ...
    g.flashmoe_decode_router_kernel_s = 0.0;
    g.flashmoe_decode_readback_wait_s = 0.0;
    g.flashmoe_decode_readback_copy_s = 0.0;
    g.flashmoe_decode_slot_lookup_s = 0.0;
    g.flashmoe_decode_blob_read_s = 0.0;
    g.flashmoe_decode_selected_write_s = 0.0;
    g.flashmoe_decode_slot_fill_s = 0.0;
    g.flashmoe_decode_router_kernel_max_s = 0.0;
    g.flashmoe_decode_readback_wait_max_s = 0.0;
    g.flashmoe_decode_readback_copy_max_s = 0.0;
    g.flashmoe_decode_slot_lookup_max_s = 0.0;
    g.flashmoe_decode_blob_read_max_s = 0.0;
    g.flashmoe_decode_selected_write_max_s = 0.0;
    g.flashmoe_decode_slot_fill_max_s = 0.0;
    g.flashmoe_decode_router_kernel_max_layer = 0u;
    g.flashmoe_decode_readback_wait_max_layer = 0u;
    g.flashmoe_decode_readback_copy_max_layer = 0u;
    g.flashmoe_decode_slot_lookup_max_layer = 0u;
    g.flashmoe_decode_blob_read_max_layer = 0u;
    g.flashmoe_decode_selected_write_max_layer = 0u;
    g.flashmoe_decode_slot_fill_max_layer = 0u;
    ...
    if (flashmoe_timing_enabled() && g.flashmoe_decode_used) {
        ...
        if (g.flashmoe_decode_layers != 0) {
            const double inv_layers = 1.0 / (double)g.flashmoe_decode_layers;
            fprintf(stderr,
                    "ds4: flashmoe decode breakdown avg_ms_per_layer router_kernel=%.3f readback_wait=%.3f readback_copy=%.3f slot_lookup=%.3f blob_read=%.3f selected_write=%.3f slot_fill=%.3f\n",
                    g.flashmoe_decode_router_kernel_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_readback_wait_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_readback_copy_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_slot_lookup_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_blob_read_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_selected_write_s * 1000.0 * inv_layers,
                    g.flashmoe_decode_slot_fill_s * 1000.0 * inv_layers);
            fprintf(stderr,
                    "ds4: flashmoe decode breakdown max_ms_per_layer router_kernel=%.3f@L%u readback_wait=%.3f@L%u readback_copy=%.3f@L%u slot_lookup=%.3f@L%u blob_read=%.3f@L%u selected_write=%.3f@L%u slot_fill=%.3f@L%u\n",
                    g.flashmoe_decode_router_kernel_max_s * 1000.0,
                    g.flashmoe_decode_router_kernel_max_layer,
                    g.flashmoe_decode_readback_wait_max_s * 1000.0,
                    g.flashmoe_decode_readback_wait_max_layer,
                    g.flashmoe_decode_readback_copy_max_s * 1000.0,
                    g.flashmoe_decode_readback_copy_max_layer,
                    g.flashmoe_decode_slot_lookup_max_s * 1000.0,
                    g.flashmoe_decode_slot_lookup_max_layer,
                    g.flashmoe_decode_blob_read_max_s * 1000.0,
                    g.flashmoe_decode_blob_read_max_layer,
                    g.flashmoe_decode_selected_write_max_s * 1000.0,
                    g.flashmoe_decode_selected_write_max_layer,
                    g.flashmoe_decode_slot_fill_max_s * 1000.0,
                    g.flashmoe_decode_slot_fill_max_layer);
        }
    }
    ds4_log(stderr,
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (graph_backend) {
        ...
        const bool flashmoe_lazy_model = e->flashmoe_manifest_ready;
        const bool map_ok = flashmoe_lazy_model
            ? ds4_gpu_set_model_map_lazy(e->model.map, e->model.size) != 0
